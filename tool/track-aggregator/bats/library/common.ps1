...
function Set-ResultCellColorByWord {
    param (
        $ResultRange,
        [string]$CompareValue
    )
...
function Set-CompareTotalCellColor {
    param (
        $ResultRange,
        [int]$TotalRowIndex,
        [int]$BlockSize = 9,
    ...
    $sheet = $ResultRange.Worksheet

    $startRow = $ResultRange.Row
    $endRow   = $ResultRange.Row + $ResultRange.Rows.Count - 1
    $startCol = $ResultRange.Column
    $endCol   = $ResultRange.Column + $ResultRange.Columns.Count - 1
...
function Export-RangeToFile {
    ...
    if ($values -isnot [System.Array]) {
        # 1セルだけの場合
        $v = $values
        if ($v -is [double] -and $Range.NumberFormat -match "yy") {
            $v = [datetime]::FromOADate($v).ToString("yyyy/MM/dd")
        }
        $content = if ($null -eq $v) { "" } else { [string]$v }
    }
    else {
