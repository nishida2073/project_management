...
| `bats\library\common.ps1` | 4つの`.ps1`が共通で使う関数（フォルダ構成のツリー表示、Azure CLI/Microsoft Graph関連の処理）。各`.ps1`の先頭でドットソースして読み込まれる。ユーザーが直接実行するものではない |
| `bats\download-folder.ps1` / `generate-package.ps1` / `generate-config.ps1` / `upload-folder.ps1` | 各段階の実装本体（`.bat`から呼び出される。ユーザーが直接実行するものではない） |
...
> 個別ファイルのダウンロードに失敗しても処理全体は中断せず、そのファイルはログにエラーとして記録した上で続行する。
...
> 個別ファイルのアップロードに失敗しても（リトライ上限を超えた場合も含む）処理全体は中断せず、そのファイルはログにエラーとして記録した上で続行する。
...
- 各チェックボックスの右にある「開く」リンクをクリックすると、その段階に関連するフォルダ/ページを開ける。「1. ファイルダウンロード」は`DOWNLOAD_LOCAL_PATH`のローカルフォルダ、「2. 個別パッケージの作成」は`GENERATE_OUTPUT_PATH`のローカルフォルダをエクスプローラーで開く。「3. ファイルアップロード」は`UPLOAD_SITE_URL`/`UPLOAD_SITE_PATH`から組み立てたURLをブラウザで開く（アップロード先のSharePointフォルダの表示画面に遷移する）。いずれも現在選択中のクライアントの値を反映する
