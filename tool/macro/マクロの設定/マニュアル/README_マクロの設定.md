...
「システム用」シートには、以下の4つの表があります。いずれも表の**タイトル文字列**（`範囲マッピング`／`項目マッピング`／`変換マッピング-区分1`／`変換マッピング-区分2`）をキーに位置を探しに行き、その**1行下**にある列見出しを読み取ります。そのため、**タイトル・列見出しの文言と並び順は変更しないでください**（表全体を別の行・列へ動かしたり、行を追加・削除するのは問題ありません）。「範囲マッピング」「項目マッピング」の2つは、読み込み後は1つの項目名辞書にまとめられます。同じ項目名が両方の表にあるとエラーになるため、項目名が重複しないようにしてください。
