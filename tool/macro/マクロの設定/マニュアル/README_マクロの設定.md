...
「システム用」シートには、以下の3つの表があります。いずれも見出し文字列をキーにVBA側（`Cells.Find`）が位置を探しに行くため、**見出しの文言・列の並び順（見出しの右2列が「実績シート」「計算算定シート」の順であること）は変更しないでください**。

#### 項目マッピング

「年度」「案件ID」など、計画算定シート・実績シート双方の列位置（列記号）や、データ開始行を定義します。
...
- `対象範囲-開始行`：各シートでデータが始まる行番号（実績シートは2行目、計画算定シート＝計算算定シートは5行目から）
- `対象範囲-開始列`／`対象範囲-終了列`：実績反映時に貼り付ける範囲の開始・終了列
- 実績シート側に列が存在しない項目（`案件名`・`予実`）は空欄のままにします
- `区分1`／`区分2`は、実績シート側の「区分」「原価区分ID」相当の列と、計算算定シート側の「会計区分1」「会計区分2」相当の列を対応付けるためのものです（下記データマッピング表とセットで使われます）

#### データマッピング-区分1

`区分1`の、実績シート側の値を計算算定シート側の値へ変換するための対応表です。
...
`区分2`の、実績シート側の値を計算算定シート側の値へ変換するための対応表です。
