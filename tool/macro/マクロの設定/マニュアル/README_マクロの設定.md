...
`原価管理シート.xlsm`のマクロ（`原価管理シート_Module1.bas`）が参照する設定シート「ツール設定用」を管理するためのブックです。このファイルを編集してから`import-macros.bat`を実行することで、マクロのコードと一緒にこのシートも対象`.xlsm`へ反映されます。

### 設定内容

「ツール設定用」シートには、以下の4つの表があります。いずれも表の**タイトル文字列**（`範囲マッピング`／`項目マッピング`／`変換マッピング-区分1`／`変換マッピング-区分2`）をキーに位置を探しに行き、その**1行下**にある列見出しを読み取ります。そのため、**タイトル・列見出しの文言と並び順は変更しないでください**（表全体を別の行・列へ動かしたり、行を追加・削除するのは問題ありません）。「範囲マッピング」「項目マッピング」の2つは、読み込み後は1つの項目名辞書にまとめられます。同じ項目名が両方の表にあるとエラーになるため、項目名が重複しないようにしてください。
...
- `区分1`／`区分2`は、計画算定シート側の「会計区分1」「会計区分2」相当の列と、実績シート側の「区分」「原価区分ID」相当の列を対応付けるためのものです（下記変換マッピング表とセットで使われます）
