...
「システム用」シートには、以下の4つの表があります。いずれも表の**タイトル文字列**（`範囲マッピング`／`項目マッピング`／`データマッピング-区分1`／`データマッピング-区分2`）をキーに位置を探しに行き、その**1行下**にある列見出しを読み取ります。そのため、**タイトル・列見出しの文言と並び順は変更しないでください**（表全体を別の行・列へ動かしたり、行を追加・削除するのは問題ありません）。「範囲マッピング」「項目マッピング」の2つは、読み込み後は1つの項目名辞書にまとめられます。同じ項目名が両方の表にあるとエラーになるため、項目名が重複しないようにしてください。

#### 範囲マッピング

各シートでデータが始まる行や、実績反映時に貼り付ける範囲の開始・終了列を定義します。
...
- `開始行`：各シートでデータが始まる行番号（実績シートは2行目、計画算定シート＝計算算定シートは5行目から）
- `開始列`／`終了列`：実績反映時に貼り付ける範囲の開始・終了列
- **「項目名」列の値（`開始行`・`開始列`・`終了列`）は変更しないでください**

#### 項目マッピング

「年度」「案件ID」などの項目ごとに、計画算定シート側・実績シート側それぞれの列記号を対応付けます。
...
- **「項目名」列の値（`年度`・`案件ID`・`案件名`・`区分1`・`区分2`・`予実`など）は変更しないでください**（行の順序や、行の追加は問題ありません）
