...
- 送信状況 ⬜ 未・✅ 済（自動）・☑️ 済（手動）：チェックした送信状況のSMSだけに絞り込みます（未送信のみ既定でチェック済み）。いずれもチェックしない場合は送信状況による絞り込みを行いません。
...
- 形式：「🆗 正常」「🆖 異常」「❎ 除外」それぞれのチェックボックスを初期状態でONにするかどうかを設定します（いずれも既定はOFF）
