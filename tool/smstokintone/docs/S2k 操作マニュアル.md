...
基本の設定:
- 送信先名：一覧や履歴で表示される名前
- 送信先の振り分け：この送信先を使う条件となる、会社名に含まれる文字列。本文から抽出した会社名にこの文字列が含まれるかどうかで判定します。「+」で行を追加、「×」で行を削除でき、複数指定できます（いずれか1つでも含まれていれば一致とみなします）。空欄の場合は、どの文字列にも一致しなかったときのデフォルトの送信先として扱われます。複数の送信先の条件に同時に一致した場合は、その全ての送信先へ登録されます
...
　本文から抽出した会社名に適用する変換を、アプリ全体（すべての送信先）に共通する設定として指定できます。送信先の振り分け・送信元情報への登録・ログの表示・kintoneへの送信のすべてに反映されます。

- 自動変換：有効にすると、英数字を半角大文字に、それ以外の文字を全角に統一する変換を適用します
...
- 送信先名：現在の会社名の内容を、現在の送信先のルール（[送信先の設定画面](#送信先の設定画面)の「送信先の振り分け」のキーワードと会社名との一致）に照らして判定した結果を表示します（読み取り専用）。一致する送信先が無い場合は「なし」と表示されます
