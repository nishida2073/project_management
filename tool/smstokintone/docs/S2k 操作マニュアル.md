...
- 端末の権限の案内表示
- 検索条件の表示・非表示の切り替え
- 検索条件（受信日の範囲、送信先、送信状況（未送信／済（自動）／済（手動））、抽出状況（正常／異常／継続））
- 検索
- 全選択／選択解除
- SMSの一覧表示
- SMS一覧の更新
...
- 抽出状況「正常」・「異常」・「継続」：チェックした抽出状況のSMSだけに絞り込みます（本文から会社名・氏名を抽出できなかった場合が「異常」、過去に届いた抽出状況が正常なSMSから会社名・氏名・送信先を引き継いだ場合が「継続」。引き継ぎについては[アプリの設定画面](#アプリの設定画面)の「SMSの引き継ぎ」を参照）。いずれもチェックしない場合は抽出状況による絞り込みを行いません。
...
- 送信状況（未送信・自動送信済み・手動送信済み・自動返信済みをそれぞれ異なるマークで表示）、本文の抽出状況（正常・異常・継続をそれぞれ異なるマークで表示）、送信先名（送信先の設定に一致した場合・一致しない＝送信先が未設定の場合・過去のSMSから引き継いだ場合をそれぞれ異なるマークで表示）。マークの意味は一覧の上に表示される凡例で確認できます
- 受信日時と送信元
- 本文全体
- すでに送信済みのSMSは、自動送信なら青系、手動送信ならアンバー系の背景色でもハイライトされます

　送信先が未設定のSMSはチェックボックスで選択できません。本文の抽出状況が異常、または抽出状況が継続（引き継ぎによるもの）のSMSについては、それぞれチェックボックスで選択できるかどうかを[アプリの設定画面](#アプリの設定画面)の「SMS選択の対象」で切り替えられます。
...
- ログの種別（受信完了／送信開始／送信完了／返信完了）と記録日時、本文の抽出結果（正常・異常・継続をそれぞれ異なるマークで表示）。送信開始・送信完了のログのみ末尾に自動・手動のマークが付きます。SMSの送信時に会社名の変換を行った場合や、返信完了のログにもそれぞれマークが付きます
...
- 抽出状況：「正常」「異常」「継続」それぞれのチェックボックスを初期状態でONにするかどうかを設定します
...
- 「抽出状況が継続」をONにすると、抽出状況が継続（引き継ぎによるもの）のSMSも自動送信の対象に含めます。ONの場合、継続のSMSは通常どおり自動送信されます。OFFにすると自動送信をスキップし、その旨がログに記録されます
...
　同一の送信元から過去に一度でも会社名・氏名の抽出に成功したSMSが届いていれば、それ以降に届く単体では解釈できないSMS（例：会社名・氏名の記載が無い追加の連絡など）についても、本文単体の抽出結果に関わらず、その直近1件から会社名・氏名・送信先を引き継いで登録します（本文は今回届いたSMSそのもの）。引き継ぎによって処理されたSMSは、一覧やログで通常の正常・異常とは別の「継続」として表示されます。「有効にする」は、この機能自体のON／OFFを切り替えます。OFFにすると、常に今回のSMS本文だけで判定します。
