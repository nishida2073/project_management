...
　本文の抽出が正常に行われたエントリでは、送信した会社名（会社名の変換を行った場合は変換後の値）・氏名・本文を表示します。<br>
...
- 会社名の抽出
  - 本文の1行目を「会社名」または「氏名」として扱うかを設定します。
- 会社名の変換
  - 抽出後の会社名の変換ルールを設定します。
  - 自動変換
    - 有効にすると、英数字を半角大文字に、それ以外の文字を全角に統一する変換を適用します
  - 固定変換
...
　送信元ごとに1件の引継ぎ内容が表示されます。各引継ぎ内容には以下が含まれます。<br>
