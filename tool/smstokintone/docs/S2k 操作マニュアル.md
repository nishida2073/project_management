...
　送信モードは[アプリの設定画面](#アプリの設定画面)の「送信モード」、返信モードは同画面の「返信モード」で選んだ設定と連動しており、互いに独立して設定できます。
...
　権限の許可は[アプリの設定画面](#アプリの設定画面)の「端末の許可」から行います。許可すると、この画面を開き直したときに通常の画面に切り替わります。
...
　画面を開いた直後の表示・非表示は、[アプリの設定画面](#アプリの設定画面)の「検索条件の表示」設定に従います。
...
- 抽出状況「正常」・「異常」・「未実施」：チェックした抽出状況のSMSだけに絞り込みます（本文から会社名・氏名を抽出できなかった場合が「異常」、過去に届いた抽出状況が正常なSMSから会社名・氏名・送信先を引き継いだ場合が「未実施」。引き継ぎについては[アプリの設定画面](#アプリの設定画面)の「SMSの引き継ぎ」を参照）。いずれもチェックしない場合は抽出状況による絞り込みを行いません。
...
　送信先が未設定のSMSはチェックボックスで選択できません。本文の抽出状況が異常、または抽出状況が未実施（引き継ぎによるもの）のSMSについては、それぞれチェックボックスで選択できるかどうかを[アプリの設定画面](#アプリの設定画面)の「SMS選択の対象」で切り替えられます。
...
　一覧の表示順（受信日時が新しい順）に関わらず、送信自体は受信日時が古いものから順に、バックグラウンドで1件ずつ処理されます。
...
　ログは新しいものから順に、1件ずつ以下の内容が表示されます。マークの意味は一覧の上に表示される凡例で確認できます。
...
　「クリア」ボタンを押すと「ログをクリアしますか？」の確認ダイアログが表示され、「クリア」を押すとログが全件削除されます。
...
　「送信完了」のログを長押しすると、送信した会社名（会社名の変換を行った場合は変換後の値）・氏名・本文の値をダイアログで確認できます。
...
　送信先名・サブドメイン・認証情報・アプリID・フィールドコードのいずれかが未入力の送信先がある場合も、その送信先名（または番号）を示すエラーダイアログが表示され、保存は行われません。
...
　SMSの検索画面での検索に使用します。未許可の場合、SMSの検索画面は使えません。

　「SMSの読み取りを許可する」ボタンを押すと、端末の権限確認ダイアログが表示されます。
...
　SMSの自動操作（自動送信・自動返信）に使用します。

　未許可の場合、SMSを自動検知できないため自動送信・自動返信ともにできません（手動での検索・送信・返信は可能です）。

　「SMSの受信を許可する」ボタンを押すと、端末の権限確認ダイアログが表示されます。
...
　「SMSの送信を許可する」ボタンを押すと、端末の権限確認ダイアログが表示されます。
...
　自動を選ぶと、SMSを受信した時点で自動的にSMSが送信されます（[トップ画面](#トップ画面)の表示にも反映されます）。

　ただし、本文の抽出状況が異常なSMSについては「自動送信対象」の設定に応じて送信をスキップできます。
...
　「手動」「自動」から選びます（送信モードとは独立した設定です）。

　自動を選ぶと、本文の抽出状況が異常なSMSを受信した際に、送信元へ「抽出状況が異常な場合」の返信メッセージを自動でSMS送信します。
...
　自動返信が有効な場合に、同じ送信元へ再度自動返信するまでの最短間隔を秒単位で指定します。
...
　「制限なし」（同一送信元なら日付を問わず常に引き継ぐ）と「同一日のみ」（日付が変わると引き継ぎがリセットされる）から選びます。
...
　引き継ぎによるSMSの送信元表示を、電話番号の代わりに引き継いだ氏名にするかどうかを切り替えます。
...
　「送信元情報の編集」を押すと[送信元情報の編集画面](#送信元情報の編集画面)が開きます。
...
　SMS本文から会社名・氏名を抽出する際に、通常のルールベースの解析の代わりに端末上のAI（Gemini Nano）を使用することができます。
...
　送信元と受信日時の近さでSMSと突き合わせをする際の許容誤差（秒）です。
...
　[ログの一覧画面](#ログの一覧画面)の一覧に表示する本文の最大文字数です。

　ここで指定した文字数を超える部分は表示されません。
...
　各送信元情報の「削除」ボタンを押すと、その送信元情報が一覧から外れます。
...
　「すべて削除」ボタンを押すと、表示中の全送信元情報が一覧から外れます。
...
　「設定を保存」ボタンを押すと、画面を開いてからの変更（会社名・氏名の編集、削除）を反映します。

　会社名・氏名のどちらかが空欄の送信元情報が残っている場合は保存されず、該当する送信元を示す入力エラーのダイアログが表示されます。
