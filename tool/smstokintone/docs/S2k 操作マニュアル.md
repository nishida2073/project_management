...
- 送信モード
  - 自動：受信したSMSを自動でkintoneに送信する状態
  - 手動：SMSの検索画面から選んで送信する状態
- 返信モード
  - 自動：抽出状況が異常なSMSを受信した際に、自動でSMSを返信する状態
-   手動：SMSの検索画面から選んで送信する状態
...
- SMSの検索
  - [SMSの検索画面](#SMSの検索画面)に移動する
- ログの確認
  -[ログの一覧画面](#ログの一覧画面)に移動する
- 送信先の設定
  - [送信先の設定画面](#送信先の設定画面)に移動する
- アプリの設定
  - [アプリの設定画面](#アプリの設定画面)に移動する
- 設定のインポート・エクスポート
  - [設定のインポート・エクスポート画面](#設定のインポート・エクスポート画面)に移動する

<div class="pagebreak"></div>

### SMSの検索画面
　受信済みのSMSの内容を確認し、手動でSMSを送信するための画面です。
...
###### 画面の項目 {#sms_search_filter_toggle-items}
- 表示／非表示の切り替え
  - ▼表示
  - ▲隠す
...
###### 画面の項目 {#app_settings_permission_settings-items}
- SMSの読み取り
- SMSの受信
- SMSの送信

> **注意**
> - 許可されている場合は「許可済み」と表示され、ボタンと説明文は隠れ非表示になります。
...
　画面上部の検索欄に、キーワードを入力すると、入力した文字が含まれる送信元情報だけが一覧に表示されます。
...
> **注意**
> - キーワードは大文字小文字・半角全角を区別しません。
>   - 「ＡＢＣ」と「abc」は同じものとして扱います。
