 ...
 *
 * 自動送信では受信時点でSMSプロバイダ上の実IDを確実に特定する手段がない（電話番号の表記ゆれや
 * SMSCタイムスタンプと端末時計のずれで一致に失敗する）ため、実IDを事前に解決して保存するのではなく、
 * 送信元とタイムスタンプの近さで都度突き合わせる方式にしている。この許容範囲はアプリの設定画面
 * （[SettingsStore.Config.smsMatchToleranceSeconds]、「自動送信」ブロックの統合範囲）で変更できる。
 *
 * 本文は判定条件には使わない（SMSCタイムスタンプと同様、プロバイダ書き込み時の改行・空白の正規化
