 ...
 * [ContinuationItemActivity]から個別の閲覧・編集・削除もできる。編集画面のように読み込みから保存
...
object ContinuationStore {
     ...
     * KintoneUploadWorker（SMS受信・送信時）とContinuationItemActivity（編集画面）が同一プロセス内から
     ...
     * 参照）、送信先の設定を変更・削除しても引き継ぎ項目側の追随作業は不要になる
     */
    data class Entry(
        /** 会社名 */
     ...
     * [senderKey]は呼び出し側で既に正規化済みであることを前提とする（引き継ぎ項目の編集画面専用）
     */
    ...
    /** 正規化済みの送信元キー[senderKey]のデータを削除する（引き継ぎ項目の編集画面専用） */
    ...
    /**
     * 引き継ぎ項目の編集画面専用。編集開始時に読み込んだ内容[expectedSnapshot]が現在の保存内容と
