...
/**
 * 継続SMS（[SettingsStore.SmsResolution.isContinuation]）の引継ぎに使う、送信元ごとの最新の
 * 抽出状況が正常なSMSの抽出結果を保持する専用のストア。SmsLogStore（全履歴のログ）とは別ファイルで
 * 管理し、ログをクリアしても引継ぎ内容は失われない。送信元ごとに最新1件のみ保持する
 * （継続SMS自体の結果は保存しない。引継ぎ元と同じ内容の再保存になり意味が無いため）。
...
object ContinuationStore {
     ...
     * 都度判定するため、送信先の設定を変更・削除しても引継ぎ内容側の追随作業は不要になる。
     *
     * @property companyName 抽出された会社名
     * @property userName 抽出された氏名
     * @property timestampMillis 引継ぎ元のSMS受信/送信対象日時（ミリ秒）
    ...
    /**
     * 引継ぎ内容の読み書きに使うSharedPreferencesインスタンスを取得する。
     *
     ...
     * 引継ぎ内容の編集画面（[ContinuationInfoSettingsActivity]）専用。
     *
     * @param context アプリケーションコンテキスト
     * @param senderKey 正規化済みの送信元キー
     * @param entry 保存する引継ぎ内容
     */
     ...
     * 継続SMS引継ぎ有効判定（[SettingsStore.ContinuationScope]）で使用。
     *
     * @param context アプリケーションコンテキスト
     * @param sender 送信元アドレス（電話番号など）
     * @param timestampMillis 判定対象のSMS日時（ミリ秒）
     * @param sameDayOnly 同一暦日に限定する場合true
     * @return 引継ぎ内容。該当なしはnull
     */
    ...
    /**
     * 正規化した送信元キーをキーとする全引継ぎ内容のマップを返す。
     *
     * @param context アプリケーションコンテキスト
     * @return 送信元キー→引継ぎ内容のマップ
     */
    ...
    /**
     * 引継ぎ内容マップをJSON配列として保存し直す。
     *
     * @param context アプリケーションコンテキスト
     * @param entries 保存対象の引継ぎ内容マップ（正規化済みキー）
     */
